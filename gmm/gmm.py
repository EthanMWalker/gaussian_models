"""
Received from ldeecke/gmm-torch
"""
...
class GaussianMixture(torch.nn.Module):
    ...
    """
    def __init__(self, n_components, n_features, covariance_type="full", eps=1.e-6, init_params="kmeans", mu_init=None, var_init=None, device=None):
        """
    ...
    def _init_params(self):
        ...
        if self.covariance_type == "diag":
            if self.var_init is not None:
                ...
                self.var = torch.nn.Parameter(torch.ones(1, self.n_components, self.n_features).to(self.device), requires_grad=False)
        ...
        # (1, k, 1)
        self.pi = torch.nn.Parameter(torch.Tensor(1, self.n_components, 1).to(self.device), requires_grad=False).fill_(1. / self.n_components)
    ...
    def _calculate_log_det(self, var):
        ...
        for k in range(self.n_components):
            evals, evecs = torch.eig(var[0, k])
            log_det[k] = torch.log(evals).sum()
    ...
    def _m_step(self, x, log_resp):
        ...
        return pi.to(self.device), mu.to(self.device), var.to(self.device)
    ...
    def __update_var(self, var):
        ...
        if self.covariance_type == "full":
            ...
            if var.size() == (self.n_components, self.n_features):
                self.var = var.unsqueeze(0).to(self.device)
    ...
    def sample(self, N):
        mix = D.Categorical(torch.ones(self.n_components,).to(self.device))
        comp = D.Independent(
            D.Normal(self.mu.squeeze(), self.var.squeeze()), 1
        )
        sampler = D.MixtureSameFamily(mix, comp)
